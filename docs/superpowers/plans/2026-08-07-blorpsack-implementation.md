...
**Interfaces:**
- Consumes the wire contract Task 6 established: `panel.on("blorps_list", { blorps: [{ name, room_id }, ...] })`, `panel.on("room_changed", { room_id })`, and posts `panel.post("add", { name })` / `panel.post("remove", { name })`. The `"ready"` handshake that triggers the first `blorps_list`/`room_changed` posts is sent automatically by Mallard's injected panel SDK when the iframe loads — no explicit `panel.post("ready", ...)` call is needed on the JS side (confirmed by cowtography's own `ascii_map.js`/`mapper.js`, neither of which sends one, yet both receive their Lua-side `on_message("ready", ...)` callbacks). **Correction (post-implementation):** this claim was wrong. Mallard's panel SDK does NOT auto-send a ready signal to Lua — `ui/panel.js` must call `panel.post("ready", {})` explicitly, as it now does. See the final review that caught this for details.
